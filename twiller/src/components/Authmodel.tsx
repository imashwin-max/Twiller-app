...
export default function AuthModal({ isOpen, onClose, initialMode = 'login' }: AuthModalProps) {
  const { login, signup, isLoading, completeLoginWithOtp } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (result?.blocked) {
          setBlockedMessage(result.message || "Login blocked.");
          return;
        }
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await completeLoginWithOtp(loginEmail, loginOtp);
      onClose();
      setFormData({ email: '', password: '', username: '', displayName: '' });
      setErrors({});
    } catch {
        ...
        <CardContent className="space-y-6">
          {blockedMessage && (
            <div className="bg-red-900/30 border border-red-700 rounded-lg p-3 mb-4">
              <p className="text-red-400 text-sm text-center">{blockedMessage}</p>
            </div>
          )}
          ...
          {requiresOtp ? (
            <form onSubmit={handleOtpSubmit} className="space-y-4">
              ...
              <Input
                ...
                maxLength={6}
                className="bg-gray-800 border-gray-700 text-white text-center tracking-widest text-lg font-mono"
              />
              <Button
                type="submit"
                disabled={loginOtp.length !== 6 || isLoading}
                className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-full py-3 text-lg"
              >
                ...
                <div className="flex justify-between items-center">
                  ...
                  {mode === "login" && (
                    <Button
                      type="button"
                      variant="link"
                      className="text-xs text-blue-400 hover:text-blue-300 p-0 h-auto font-semibold"
